...
export const state = () => ({
  ...
  export const mutations = {
    ...
    ADD (state, payload){
      console.log("VUEX - COMMENTS ADD")
      fireStore.collection('comments').add(payload)
      .then((docRef) => {
    ...
    UPDATE (state, payload) {
      console.log("VUEX - COMMENTS UPDATE")
      fireStore.collection('comments').where("number","==",payload.number).where("id","==",payload.id).get()
      .then(querySnapshot => {
          querySnapshot.forEach( doc => {
            fireStore.collection("comments").doc(doc.id).update({...payload})
              .then( () => {
    ...
    DELETE (state, payload) {
      console.log("VUEX -COMMENTS DELETE")
      fireStore.collection('comments').where('number','==',payload.number).where('id','==',payload.id).get()
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          doc.ref.delete()
