...
export const state = () => ({
  ...
  export const mutations = {
    GET (state, payload) {
      let items = []
      console.log("VUEX - MILESTONES GET")
    ...
    ADD (state, payload){
      console.log("VUEX - MILESTONES ADD")
      fireStore.collection('milestones').add(payload)
      .then((docRef) => {
    ...
    UPDATE (state, payload) {
      console.log("VUEX - MILESTONES UPDATE")
      fireStore.collection('milestones').where("number","==",payload.number).where("id","==",payload.id).get()
      .then(querySnapshot => {
          querySnapshot.forEach( doc => {
            fireStore.collection("milestones").doc(doc.id).update({...payload})
              .then( () => {
    ...
    DELETE (state, payload) {
      console.log("VUEX - MILESTONES DELETE")
      fireStore.collection('milestones').where('number','==',payload.number).where('id','==',payload.id).get()
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          doc.ref.delete()
          .then( () =>{
            ...
            Swal.fire({
              type: 'success',
              title: 'MILESTONE REMOVED SUCCESSFULLY',
