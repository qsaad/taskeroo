                ...
                <v-container grid-list-md>
                    <v-layout wrap>
                        <v-flex xs12>
                            ...
                            <v-text-field label="Number" v-model.number="editedItem.rfiNumber"></v-text-field>
                            <v-text-field label="Title" v-model="editedItem.title"></v-text-field>
                            <v-combobox label="Action" :items="rfisActionList" v-model="editedItem.action"></v-combobox>
                ...
                <v-flex xs12>
                    ...
                    >
                        <template v-slot:items="props">
                                ...
                                <td class="text-xs-left subheading green--text">
                                    {{ formatRfiNumber(props.item.rfiNumber) }}
                                </td>
                                 <td class="text-xs-left subheading green--text">
                                    {{ props.item.title }}
                                </td>
...
export default {
    ...
    mounted(){
        this.items = this.$store.getters['rfis/listByNumber'](this.number)
    },//MOUNTED
    watch: {
        dialog (val) {
            val || this.close()
    ...
    computed:{
        formTitle () {
            return this.editedIndex === -1 ? 'New RFI' : 'Edit RFI'
        },
    ...
    methods:{
        ...
        deleteItem (item) {
            ...
            Swal.fire({
                type: 'warning',
                title: 'Are you sure you want to delete this RFI?',
                text: item.url,
            ...
            .then((result) => {
                if (result.value) {
                    ...
                    this.$store.dispatch('rfis/delete',item)
                } 
            })
        },
        close () {
        ...
        save () {
            if (this.editedIndex > -1) {
                ...
                this.$store.dispatch('rfis/update',this.editedItem)
                this.editedIndex = -1
            } 
            else {
                if(_.findIndex(this.items, ['number', this.editedItem.number]) > -1){
                    Swal.fire({
                        type: 'warning',
                        title: 'RFI ' + this.editedItem.number + ' already exists on ',
                    ...
                    this.$store.dispatch('rfis/add',payload)
                }
            }
            this.editedIndex = -1
            this.close()
